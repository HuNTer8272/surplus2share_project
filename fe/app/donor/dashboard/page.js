  ...
  useEffect(() => {
    ...
    const fetchUserAndDonations = async () => {
      try {
        ...
        console.log("Fetching donations from:", `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/donations/me`);
        
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/donations/me`,
          {
            headers: {
